...
> 状态：**设计中（未落地）**，对应阶段 18–23。本文件是本轮云迁移的**唯一事实源**，
...
| 阶段 | 目标 | 依赖 |
| --- | --- | --- |
| 18 | 鉴权数据层与配置基座（`app/auth/` 骨架 + `auth.sqlite` + 配置项，不接线） | — |
| 19 | 飞书 OAuth 单管理员登录 + 会话门禁（保护 `/`、`/dashboard`、snapshot） | 18 |
| 20 | API Key 鉴权 + 管理控制台 + 自描述更新（保护 `/api/v1/android/*`） | 18、19 |
| 21 | 对象存储抽象 + 本地后端（**行为保持**重构，现有测试全绿） | — |
| 22 | GCS / S3 后端 + signed URL 302 下发 | 21 |
| 23 | 云上 Docker Compose 变体 + 部署收尾（去 NAS、env 模板、smoke、文档） | 18–22 |
