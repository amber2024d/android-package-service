...
> 状态：**实施中**——阶段 18–19 已落地（2026-07-01），阶段 20–23 待实现。本文件是本轮云迁移的**唯一事实源**，
...
| 20 | API Key 鉴权 + 管理控制台 + 自描述更新（保护 `/api/v1/android/*`） | 18、19 | ⏳ 待实现 |
| 21 | 对象存储抽象 + 本地后端（**行为保持**重构，现有测试全绿） | — | ⏳ 待实现 |
| 22 | GCS / S3 后端 + signed URL 302 下发 | 21 | ⏳ 待实现 |
| 23 | 云上 Docker Compose 变体 + 部署收尾（去 NAS、env 模板、smoke、文档） | 18–22 | ⏳ 待实现 |
