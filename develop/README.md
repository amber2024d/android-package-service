...
- [云迁移改造设计（鉴权 + 对象存储 + 云部署）](android-package-service-cloud-migration-design.md)——迁到公网云 VM：数据 API 加 API Key、首页/监控面板加飞书 OAuth（单管理员）、NAS artifact 抽象为 GCS/S3 工厂 + signed URL 下发、去 NAS 的云上 Docker Compose 变体。阶段拆分见 §2。
