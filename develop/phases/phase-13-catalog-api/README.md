...
`/download`（name | code | 空=最新）。当前 `/download` 未命中 artifact 时先返回 `202` 下载任务，由独立 worker
执行真实下载；指定版本同时后台异步补目录（fire-and-forget + 收集单飞），保证接口响应不被下载或目录收集拖慢。
...
```text
app/api/routes.py             # 新增 GET /versions；/download 接入队下载 + 后台收集
...
2. **`/download` 指定版本**：命中 artifact 直接返回；未命中时写入 `download_jobs` 返回 `202`，**同时 fire-and-forget** 触发
   `ensure_collected`（收集单飞，与 §G 定时任务共用同一把）。
3. **`/download` 不传版本**：最新版不触发收集；未命中 artifact 时同样由 download worker 执行 provider 最新版快路径。
4. worker 全链 fallback 下不到 → job failed（后台收集仍在补库）；下载成功照常回填账本（阶段 10 钩子）。
5. 响应观测：`/versions` 不在请求里触发刷新（新鲜度交阶段 14 定时任务）。

## 测试

- `/versions`：只出 downloadable；首采阻塞一次、复查读库；known-only 不出现。
- `/download` 未收集包指定版本：未缓存返回 `202` + 后台收集被触发且**不阻塞响应**；worker 下不到 → job failed。
- `/download` 最新版：不触发收集，未缓存时由 worker 跑 provider 最新版快路径。
- 并发：同 request_key 只排一个 active job；同包同版本只落一份 artifact；同包并发只一个收集任务。

## 验收标准

- `/versions` 只返回可下载版本，响应不被收集/刷新拖慢。
- `/download` 命中未收集包也能先入队，目录后台补齐。
- 同请求不重复排队、同包同版本不重复落盘、同包不重复收集。
...
- 落地：`GET /api/v1/android/apps/{packageName}/versions`（路径用既有 `/apps/{pkg}/...` 约定，
  ...
  （保强引用 + 完成回收 + 异常记 `catalog_collect_failed`），最新版不触发。后续已接入 `download_jobs` + 独立
  `app.download.worker`，未命中 artifact 返回 `202` 任务。`CatalogVersion`/`CatalogVersionsResponse` 入
  `domain/models.py`，`get_catalog` 依赖按 provider 开关装配采集器。
